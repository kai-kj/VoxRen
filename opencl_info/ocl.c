// gcc -o opencl_info/ocl opencl_info/ocl.c -lOpenCL -std=c99; ./opencl_info/ocl
...
int main(void) {
	...
	for (int platform_idx = 0; platform_idx < num_platforms_available; platform_idx++) {
		...
		// Get platform name
		err_num =
			clGetPlatformInfo(cl_platforms[platform_idx], CL_PLATFORM_NAME, sizeof(str_buffer), &str_buffer, NULL);
		...
		// Get platform vendor
		err_num =
			clGetPlatformInfo(cl_platforms[platform_idx], CL_PLATFORM_VENDOR, sizeof(str_buffer), &str_buffer, NULL);
		...
		err_num =
			clGetPlatformInfo(cl_platforms[platform_idx], CL_PLATFORM_VERSION, sizeof(str_buffer), &str_buffer, NULL);
		...
		err_num =
			clGetPlatformInfo(cl_platforms[platform_idx], CL_PLATFORM_PROFILE, sizeof(str_buffer), &str_buffer, NULL);
		...
		err_num = clGetPlatformInfo(cl_platforms[platform_idx], CL_PLATFORM_EXTENSIONS, sizeof(str_buffer), &str_buffer,
									NULL);
		...
		err_num =
			clGetDeviceIDs(cl_platforms[platform_idx], CL_DEVICE_TYPE_ALL, num_devices_available, cl_devices, NULL);
		...
		for (int device_idx = 0; device_idx < num_devices_available; device_idx++) {
			...
			err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_OPENCL_C_VERSION, sizeof(str_buffer),
									  &str_buffer, NULL);
			...
			err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(max_clock_freq),
									  &max_clock_freq, NULL);
			...
			printf("\t\t\t\t\t [Platform %d] [Device %d] CL_DEVICE_MAX_CLOCK_FREQUENCY: %d MHz\n", platform_idx,
				   device_idx, max_clock_freq);

			// Get device max compute units available
			cl_uint max_compute_units_available;
			err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_MAX_COMPUTE_UNITS,
									  sizeof(max_compute_units_available), &max_compute_units_available, NULL);
			...
			err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_GLOBAL_MEM_SIZE, sizeof(global_mem_size),
			...
			printf("\t\t\t\t\t [Platform %d] [Device %d] CL_DEVICE_GLOBAL_MEM_SIZE: %llu MB\n", platform_idx,
				   device_idx, (unsigned long long)global_mem_size / (1024 * 1024));

			// Get device max compute units available
			cl_ulong max_mem_alloc_size;
			err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_MAX_MEM_ALLOC_SIZE, sizeof(max_mem_alloc_size),
			...
			printf("\t\t\t\t\t [Platform %d] [Device %d] CL_DEVICE_MAX_MEM_ALLOC_SIZE: %llu MB\n", platform_idx,
				   device_idx, (unsigned long long)max_mem_alloc_size / (1024 * 1024));

			// Get device local mem size
			cl_ulong local_mem_size;
			err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_LOCAL_MEM_SIZE, sizeof(local_mem_size),
									  &local_mem_size, NULL);
			...
			err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_MAX_WORK_GROUP_SIZE,
			...
			printf("\t\t\t\t\t [Platform %d] [Device %d] CL_DEVICE_MAX_WORK_GROUP_SIZE: %ld\n", platform_idx,
				   device_idx, (long int)max_work_group_size);

			// Get device max work item dim
			cl_uint max_work_item_dims;
			err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
			...
			printf("\t\t\t\t\t [Platform %d] [Device %d] CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS: %d\n", platform_idx,
				   device_idx, max_work_item_dims);

			// Get device max work item sizes in each dimension
			size_t work_item_sizes[max_work_item_dims];
			err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(work_item_sizes),
			...
			err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_IMAGE_SUPPORT, sizeof(image_support),
									  &image_support, NULL);
			...
			printf("\t\t\t\t\t [Platform %d] [Device %d] CL_DEVICE_IMAGE_SUPPORT: %u (%s)\n", platform_idx, device_idx,
				   image_support, image_support ? "Available" : "Not available");
			...
			if (image_support) {
				...
				err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_IMAGE2D_MAX_WIDTH, sizeof(image_size),
										  &image_size, NULL);
				...
				printf("\t\t\t\t\t [Platform %d] [Device %d] CL_DEVICE_IMAGE2D_MAX_WIDTH: %ld\n", platform_idx,
					   device_idx, (long int)image_size);

				// Get device image 2d max height
				err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_IMAGE2D_MAX_HEIGHT, sizeof(image_size),
										  &image_size, NULL);
				check_cl_error(err_num, "clGetDeviceInfo: Getting device image max 2d width");
				...
				err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_IMAGE3D_MAX_WIDTH, sizeof(image_size),
										  &image_size, NULL);
				...
				err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_IMAGE3D_MAX_HEIGHT, sizeof(image_size),
										  &image_size, NULL);
				...
				err_num = clGetDeviceInfo(cl_devices[device_idx], CL_DEVICE_IMAGE3D_MAX_DEPTH, sizeof(image_size),
										  &image_size, NULL);
